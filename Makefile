...
.PHONY: all dvi kontinuertpdf bookletpdf clean pdf
...
kontinuertpdf: 
	...
	@echo "PDF should be generated in the $(OUTPUT_DIR)/kontinuert directory: $(OUTPUT_DIR)/kontinuert/kontinuert.pdf"

bookletpdf: 
	if ! test -f $(OUTPUT_DIR)/kontinuert/kontinuert.pdf; then $(MAKE) kontinuertpdf; fi
	@echo "Compiling .dvi to .pdf with A4 paper size"
	mkdir -p $(OUTPUT_DIR)
	mkdir -p $(OUTPUT_DIR)/booklet
	pdflatex -output-directory=$(OUTPUT_DIR)/booklet -jobname=booklet $(FILE_BOOKLET)
	@echo "PDF should be generated in the $(OUTPUT_DIR)/booklet directory: $(OUTPUT_DIR)/booklet/booklet.pdf"
